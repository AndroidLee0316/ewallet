package com.pingan.smartcity.ipark.etown.wxapi;
  ...
  @Override public void onResp(BaseResp baseResp) {
    ...
    if (baseResp.getType() == ConstantsAPI.COMMAND_LAUNCH_WX_MINIPROGRAM) {
      ...
      if (baseResp.errCode == BaseResp.ErrCode.ERR_OK) {
        PASCPay.getInstance().notifyPayResult(PayUtil.PayType.WECHATJSAPI, PayResult.PAY_SUCCESS, "");
      } else if (baseResp.errCode == BaseResp.ErrCode.ERR_USER_CANCEL) {
        PASCPay.getInstance().notifyPayResult(PayUtil.PayType.WECHATJSAPI, PayResult.PAY_CANCEL, "");
      } else if (baseResp.errCode == BaseResp.ErrCode.ERR_SENT_FAILED) {
        PASCPay.getInstance().notifyPayResult(PayUtil.PayType.WECHATJSAPI, PayResult.PAY_FAILED, baseResp.errStr);
      } else {
        PASCPay.getInstance()
            .notifyPayResult(PayUtil.PayType.WECHATJSAPI, PayResult.PAY_WAITING, baseResp.errStr);
      }
    }
  }
}