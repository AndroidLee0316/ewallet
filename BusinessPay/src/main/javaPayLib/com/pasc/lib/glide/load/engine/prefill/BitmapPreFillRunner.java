 ...
 * com.pasc.lib.glide.load.engine.bitmap_recycle.BitmapPool} is pre-populated.
 *
 * <p>By posting to the main thread with backoffs, we try to avoid ANRs when the garbage collector
 * gets into a state where a high percentage of {@link Bitmap} allocations trigger a retry the world
 ...
 * limit, we assume a GC has occurred, retry the current allocations, and try again after a delay.
 */
