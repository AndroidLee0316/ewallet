...
class HighlightView {
    ...
    private void initStyles(Context context) {
        ...
        context.getTheme().resolveAttribute(R.attr.ewalletCropImageStyle, outValue, true);
        TypedArray attributes = context.obtainStyledAttributes(outValue.resourceId, R.styleable.EwalletCropImageView);
        try {
            showThirds = attributes.getBoolean(R.styleable.EwalletCropImageView_showThirds, false);
            showCircle = attributes.getBoolean(R.styleable.EwalletCropImageView_showCircle, false);
            highlightColor = attributes.getColor(R.styleable.EwalletCropImageView_highlightColor,
                    DEFAULT_HIGHLIGHT_COLOR);
            handleMode = HandleMode.values()[attributes.getInt(R.styleable.EwalletCropImageView_ewalletShowHandles, 0)];
        } finally {
            attributes.recycle();
        }
    }
