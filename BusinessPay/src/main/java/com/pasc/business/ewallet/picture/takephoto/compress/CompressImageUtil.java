package com.pasc.business.ewallet.picture.takephoto.compress;
...
public class CompressImageUtil {
            ...
            public void run() {
                ...
                try {
                    File thumbnailFile = getThumbnailFile(new File(imgPath));
                     fos = new FileOutputStream(thumbnailFile);//将压缩后的图片保存的本地上指定路径中
     ...
     */
    private void compressImageByPixel(String imgPath, CompressListener listener) {
        if (imgPath == null) {
        ...
        if (config.isEnableQualityCompress()) {
            ...
            try{
                ...
                e.printStackTrace();
                listener.onCompressFailed(thumbnailFile.getPath(), e.toString());
            }finally {
